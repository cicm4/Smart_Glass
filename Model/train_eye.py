...
model = EyeBlinkNet(input_size=train_ds.n_pixels).to(device)
...
for epoch in range(1, 101):
    ...
    if va_f1 > best_f1 + 1e-3 and va_f1 > CURR_BEST_F1:
        best_f1 = va_f1
        no_improve = 0
        torch.save(model.state_dict(), constants.Paths.IMG_WEIGHTS)
...
if os.path.exists(constants.Paths.IMG_WEIGHTS):
    model.load_state_dict(torch.load(constants.Paths.IMG_WEIGHTS, map_location=device))
    model.eval()
