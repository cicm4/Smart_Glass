...
class BlinkDetectorLarge(nn.Module):
    def __init__(self, num_features=7, img_height=24, img_width=12,
                 conv_channels=(32, 64, 128), lstm_hidden=256,
                 lstm_layers=3, bidirectional=True, dropout=0.4):
        super().__init__()
        self.conv1 = nn.Conv2d(1, conv_channels[0], 3, padding=1)
        ...
        conv_out_h = img_height // 8   # 24 -> 3
        conv_out_w = img_width  // 8   # 12 -> 1
        flat_dim   = conv_out_h * conv_out_w * conv_channels[2]

        self.img_fc1 = nn.Linear(flat_dim, 256)
        self.img_fc2 = nn.Linear(256, 128)
        self.num_fc1 = nn.Linear(num_features, 64)
        self.num_fc2 = nn.Linear(64, 32)

        lstm_in = 128 + 32
        self.lstm = nn.LSTM(lstm_in, lstm_hidden, lstm_layers,
                            batch_first=True, bidirectional=bidirectional, dropout=dropout)
        out_dim = lstm_hidden * (2 if bidirectional else 1)
        self.fc1 = nn.Linear(out_dim, 64)
    ...
    def forward(self, eye_seq, num_seq):
        ...
        x = x.view(B*T, -1)
        x = self.relu(self.img_fc1(x))
        x = self.dropout(x)
        x = self.relu(self.img_fc2(x))

        n = self.relu(self.num_fc1(num_seq.view(B*T, -1)))
        n = self.dropout(n)
        n = self.relu(self.num_fc2(n))
        ...
        if self.lstm.bidirectional:
            seq_rep = torch.cat([h[-2], h[-1]], dim=1)
        else:
            seq_rep = h[-1]
        out = self.relu(self.fc1(seq_rep))
        out = self.dropout(out)
        return self.fc2(out).squeeze(1)

# ---------- BLOCK 3 : Training setup -----------------------------------
model = BlinkDetectorLarge(num_features=7).to(device)
...
for epoch in range(100):
    ...
    if va_f1 > best_f1 + 0.001:
        best_f1 = va_f1
        no_improve = 0
        torch.save(model.state_dict(), "blink_best_large.pth")
    else:
        no_improve += 1
...
model.load_state_dict(torch.load("blink_best_large.pth", map_location=device))
model.eval()
